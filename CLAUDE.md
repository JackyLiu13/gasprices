...
[`PREVIEW_PLAN.md`](PREVIEW_PLAN.md) is an approved plan for a browser-based
screen preview and layout editor. **Phase A/B are in progress** under
`preview/` (note: the plan says `web/`; the directory on disk is `preview/`).
Landed so far: the vendored `glcdfont.h`/`glcdfont.js` pair generated by
`preview/tools/gen_font.py`, a reworked `tests/stubs/Adafruit_ST7789.h`, and
`ui_*` columns appended to `vectors.csv` so the C++ and JS renderers read
identical display inputs instead of synthesising them.
